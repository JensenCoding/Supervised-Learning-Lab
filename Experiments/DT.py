...
# Adapted from https://github.com/cmaron/CS-7641-assignments/blob/master/assignment1/learners/DT.py
class DTLearner:
    def __init__(self,
                 criterion="gini",
                 splitter="best",
    ...
    def prune(self):
        ...
        if self._alpha <= -1:  # Early exit
            return self
        tmp_tree = self._learner.tree_
        best_score = f1_score(self.value_x, self.value_y)
        ...
        for candidate in reversed(candidates):  # Go backwards/leaves up
            ...
            score = f1_score(self.value_x, self.value_y)
...
def DT(data):
    seed = data.seed
    params = {'criterion': ['gini', 'entropy'], 'max_depth': np.arange(1, 51, 1),
              'class_weight': ['balanced', None]}
    v_param = {'max_depth': np.arange(1, 51, 1)}

    learner = DTLearner(random_state=seed)

    best_params = exp_perform(data, learner, 'DT', params, validation_params=v_param)
